...
function calculateCost() {
  ...
  if (varietySamplingRate[selectedVariety]) {
    const totalCost = Math.round(
      plantsPerAcre * varietySamplingRate[selectedVariety]
    );
    document.getElementById(
      "result"
    ).innerHTML = `<b>Plants for 1 Acre: ${Math.round(plantsPerAcre)} </b><br/>
        Price per Plant for '${selectedVariety}' variety: ₹${
      varietySamplingRate[selectedVariety]
    }<br/>
        Total Cost = ₹${totalCost}`;
  } else {
    document.getElementById("result").innerHTML =
      "Please select a valid variety.";
  }
}
